...
namespace SmartBiz.MDMAPI.API
{
    ...
    public class HR100 : IDisposable
    {
        ...
        /// <summary>
        /// Queries the hr employee bank.
        /// </summary>
        /// <param name="query">The query.</param>
        /// <param name="pageSize">Size of the page.</param>
        /// <param name="pageNumber">The page number.</param>
        /// <returns>ResultDTO&lt;HR_EMP_BANK&gt;.</returns>
        public ResultDTO<HR_EMP_BANK> QueryEMPBANK(EmpBankQuery query, int pageSize, int pageNumber)
        {
            try
            {
                using (var ctx = new MDMEntities())
                {
                    var c = ctx.HR_EMP_BANK.AsQueryable();
                    ...
                    {
                        c = c.Where(i => i.BANK_CODE == query.BankCode);
                    }
                    if (query.BranchCode != null)
                    {
                        c = c.Where(i => i.BBRANCH_CODE == query.BranchCode);
                    }

                    int totalcount = c.Count();
                    var result = c.OrderBy(i => i.EMP_NUMBER).Skip((pageNumber - 1) * pageSize).Take(pageSize).ToArray();
                    int localcount = result.Count();
                    var dto = new ResultDTO<HR_EMP_BANK>() { Result = result, TotalResultCount = totalcount, LocalResultCount = localcount };
                    return dto;
                }
            }
            catch (Exception ex)
            {
                APIErrorLog.LogAPIError("FM100", string.Format("Unexpected error has occurred on QueryEMPBANK : {0} ", ex.Message));
                return null;
            }
        }
        ...
        /// Creates the empbank.
        /// </summary>
        /// <param name="new_HR_EMP_BANK">The new_hr_ emp_ bank.</param>
        /// <returns>ApiAck.</returns>
        ...
        /// <summary>
        /// Queries the hr employee passport.
        /// </summary>
        /// <param name="query">The query.</param>
        /// <param name="pageSize">Size of the page.</param>
        /// <param name="pageNumber">The page number.</param>
        /// <returns>ResultDTO&lt;HR_EMP_PASSPORT&gt;.</returns>
        public ResultDTO<HR_EMP_PASSPORT> QueryEMPPASSPORT(EmpPassportQuery query, int pageSize, int pageNumber)
        {
            try
            {
                using (var ctx = new MDMEntities())
                {
                    var c = ctx.HR_EMP_PASSPORT.AsQueryable();

                    if (query.EmpNumber != null)
                    {
                        c = c.Where(i => i.EMP_NUMBER == query.EmpNumber);
                    }
                    if (query.SequenceNum != null)
                    {
                        c = c.Where(i => i.EP_SEQNO == query.SequenceNum);
                    }
                    if (query.passportNum != null)
                    {
                        c = c.Where(i => i.EP_PASSPORTNUMBER == query.passportNum);
                    }

                    int totalcount = c.Count();
                    var result = c.OrderBy(i => i.EMP_NUMBER).Skip((pageNumber - 1) * pageSize).Take(pageSize).ToArray();
                    int localcount = result.Count();
                    var dto = new ResultDTO<HR_EMP_PASSPORT>() { Result = result, TotalResultCount = totalcount, LocalResultCount = localcount };
                    return dto;
                }
            }
            catch (Exception ex)
        ...
        /// <summary>
        /// Execute the query and return Basic Salary records
        /// </summary>
        /// <param name="query">The query.</param>
        /// <param name="pageSize">Size of the page.</param>
        /// <param name="pageNumber">The page number.</param>
        /// <returns>ResultDTO&lt;HR_EMP_BASICSALARY&gt;.</returns>
        public ResultDTO<HR_EMP_BASICSALARY> QueryBasicSalary(BasicSalaryQuery query, int pageSize, int pageNumber)
        {
            try
            {
                using (var ctx = new MDMEntities())
                {
                    var c = ctx.HR_EMP_BASICSALARY.AsQueryable();
                    if (query.EmployeeNumber != null)
                    {
                        c = c.Where(i => i.EMP_NUMBER == query.EmployeeNumber);
                    }
                    if (query.Year != null)
                    {
                        c = c.Where(i => i.SAL_DTL_YEAR == query.Year);
                    }
                    if (query.SalaryGradeCode != null)
                    {
                        c = c.Where(i => i.SAL_GRD_CODE == query.SalaryGradeCode);
                    }
                    if (query.CurrencyID != null)
                    {
                        c = c.Where(i => i.CURRENCY_ID == query.CurrencyID);
                    }

                    int totalcount = c.Count();
                    var result = c.OrderBy(i => i.EMP_NUMBER).Skip((pageNumber - 1) * pageSize).Include(h => h.HR_EMPLOYEE).Include(s=>s.HR_SALARY_GRADE).Include(m=>m.HR_CURRENCY).Take(pageSize).ToArray();
                    int localcount = result.Count();
                    var dto = new ResultDTO<HR_EMP_BASICSALARY>() { Result = result, TotalResultCount = totalcount, LocalResultCount = localcount };
                    return dto;
                }
            }
            catch (Exception ex)
        ...
        /// <summary>
        /// Create a new Basic Salary record
        /// </summary>
        /// <param name="new_Training">The new_Basic_Salary.</param>
        /// <returns>ApiAck.</returns>
        public ApiAck CreateBasicSalary(HR_EMP_BASICSALARY new_BasicSalary)
        {
            try
            {
                using (var transaction = new TransactionScope())
                {
                    using (var ctx = new MDMEntities())
                    {
                        var emp_number = ctx.HR_EMPLOYEE.Find(new_BasicSalary.HR_EMPLOYEE.EMP_NUMBER);
                        var sal_grd_code = ctx.HR_SALARY_GRADE.Find(new_BasicSalary.HR_SALARY_GRADE.SAL_GRD_CODE);
                        var currency_id = ctx.HR_CURRENCY.Find(new_BasicSalary.HR_CURRENCY.CURRENCY_ID);
                        new_BasicSalary.HR_EMPLOYEE = emp_number;
                        new_BasicSalary.HR_SALARY_GRADE = sal_grd_code;
                        new_BasicSalary.HR_CURRENCY = currency_id;
                        ctx.HR_EMP_BASICSALARY.Add(new_BasicSalary);
                        ctx.SaveChanges();
                    }
                    transaction.Complete();
                }
            ...
            {
                APIErrorLog.LogAPIError("FM100", string.Format("Unexpected error has occurred on creating new Basic Salary : {0} ", ex.Message));
                return new ApiAck { CallStatus = EApiCallStatus.Error, ReturnedMessage = ex.GetBaseException().Message };
            }
        }

        /// <summary>
        /// Return a Basic Salary record
        /// </summary>
        /// <param name="emp_number">The emp_number.</param>
        /// <param name="tn_id">The tn_id.</param>
        /// <returns>HR_EMP_TRAININGS.</returns>
        public HR_EMP_BASICSALARY ReadBasicSalaryByKey(string emp_number, int sal_dtl_year,string sal_grd_code,string currency_id)
        {
            try
            {
                using (var ctx = new MDMEntities())
                {
                    return ctx.HR_EMP_BASICSALARY.Where(c => c.EMP_NUMBER == emp_number && c.SAL_DTL_YEAR==sal_dtl_year && c.SAL_GRD_CODE==sal_grd_code && c.CURRENCY_ID==currency_id).FirstOrDefault();
                }
            }
            catch (Exception ex)
            {
                APIErrorLog.LogAPIError("FM100", string.Format("Unexpected error has occurred on read Basic Salary by key : {0} ", ex.Message));
                return null;
            }
        }

        /// <summary>
        /// Return all Basic Salary records
        /// </summary>
        /// <returns>HR_EMP_TRAININGS[].</returns>
        public HR_EMP_BASICSALARY[] ReadAllBasicSalary()
        {
            try
            {
                using (var ctx = new MDMEntities())
                {
                    return ctx.HR_EMP_BASICSALARY.ToArray();
                }
            }
            catch (Exception ex)
            {
                APIErrorLog.LogAPIError("FM100", string.Format("Unexpected error has occurred on read all Basic Salary : {0} ", ex.Message));
                return new HR_EMP_BASICSALARY[0];
            }
        }

        /// <summary>
        /// Update Basic Salary record
        /// </summary>
        /// <param name="modified_Training">The modified_ training.</param>
        /// <returns>ApiAck.</returns>
        public ApiAck UpdateBasicSalary(HR_EMP_BASICSALARY modified_BasicSalary)
        {
            try
            {
                using (var transaction = new TransactionScope())
                {
                    using (var ctx = new MDMEntities())
                    {
                        HR_EMP_BASICSALARY original_BasicSalary = ctx.HR_EMP_BASICSALARY.Find(modified_BasicSalary.EMP_NUMBER, modified_BasicSalary.SAL_DTL_YEAR, modified_BasicSalary.SAL_GRD_CODE, modified_BasicSalary.CURRENCY_ID);

                        if (original_BasicSalary != null)
                        {
                            ctx.Entry(original_BasicSalary).CurrentValues.SetValues(modified_BasicSalary);
                            ctx.SaveChanges();
                        }
                        else
                        {
                            ...
                            {
                                CallStatus = EApiCallStatus.ValidationFailed,
                                ReturnedMessage = string.Format("Basic Salary with employee number:{0} year:{1} salary grade code:{2} currency ID:{3} was not found.", modified_BasicSalary.EMP_NUMBER,modified_BasicSalary.SAL_DTL_YEAR,modified_BasicSalary.SAL_GRD_CODE,modified_BasicSalary.CURRENCY_ID)
                            };
                        }
                    }
                    transaction.Complete();
            ...
            {
                APIErrorLog.LogAPIError("FM100", string.Format("Unexpected error has occurred on update Basic Salary: {0} ", ex.Message));
                return new ApiAck { CallStatus = EApiCallStatus.Error, ReturnedMessage = ex.GetBaseException().Message };
            }
        }

        /// <summary>
        /// Delete BasicSalary record
        /// </summary>
        /// <param name="delete_Training">The delete_ training.</param>
        /// <returns>ApiAck.</returns>
        public ApiAck DeleteBasicSalary(HR_EMP_BASICSALARY delete_BasicSalary)
        {
            try
            {
                using (var transaction = new TransactionScope())
                {
                    using (var ctx = new MDMEntities())
                    {
                        HR_EMP_BASICSALARY original_BasicSalary = ctx.HR_EMP_BASICSALARY.Find(delete_BasicSalary.EMP_NUMBER, delete_BasicSalary.SAL_DTL_YEAR, delete_BasicSalary.SAL_GRD_CODE, delete_BasicSalary.CURRENCY_ID);

                        if (original_BasicSalary != null)
                        {
                            ctx.HR_EMP_BASICSALARY.Remove(original_BasicSalary);
                            ctx.SaveChanges();
                        }
                        else
                        {
                            ...
                            {
                                CallStatus = EApiCallStatus.ValidationFailed,
                                ReturnedMessage = string.Format("Basic Salary with employee number:{0} year:{1} salary grade code:{2} currency ID:{3} was not found.",delete_BasicSalary.EMP_NUMBER,delete_BasicSalary.SAL_DTL_YEAR,delete_BasicSalary.SAL_GRD_CODE,delete_BasicSalary.CURRENCY_ID)
                            };
                        }
                    }
                    transaction.Complete();
            ...
            {
                APIErrorLog.LogAPIError("FM100", string.Format("Unexpected error has occurred on delete Basic Salary : {0} ", ex.Message));
                return new ApiAck { CallStatus = EApiCallStatus.Error, ReturnedMessage = ex.GetBaseException().Message };
            }
        }

        /// <summary>
        /// Exists the Basic Salary.
        /// </summary>
        /// <param name="exist_Training">The exist_ training.</param>
        /// <returns><c>true</c> if XXXX, <c>false</c> otherwise.</returns>
        public bool ExistBasicSalary(HR_EMP_BASICSALARY exist_BasicSalary)
        {
            try
            {
                using (var ctx = new MDMEntities())
                {
                    HR_EMP_BASICSALARY original_BasicSalary = ctx.HR_EMP_BASICSALARY.Find(exist_BasicSalary.HR_EMPLOYEE.EMP_NUMBER, exist_BasicSalary.SAL_DTL_YEAR, exist_BasicSalary.HR_SALARY_GRADE.SAL_GRD_CODE, exist_BasicSalary.HR_CURRENCY.CURRENCY_ID);
            ...
            {
                APIErrorLog.LogAPIError("FM100", string.Format("Unexpected error has occurred Basic Salary : {0} ", ex.Message));
                return false;
            }
        }
        ...
        /// <summary>
        /// Execute the query and return Reporting records
        /// </summary>
        /// <param name="query">The query.</param>
        /// <param name="pageSize">Size of the page.</param>
        /// <param name="pageNumber">The page number.</param>
        ...
        public ResultDTO<HR_EMP_REPORTTO> QueryReporting(ReportingQuery query, int pageSize, int pageNumber)
        {
            try
            {
                using (var ctx = new MDMEntities())
                {
                    var c = ctx.HR_EMP_REPORTTO.AsQueryable();
                    if (query.SuperiorEmployeeNumber != null)
                    {
                        c = c.Where(i => i.EREP_SUP_EMP_NUMBER == query.SuperiorEmployeeNumber);
                    }
                    if (query.SubordinateEmployeeNumber != null)
                    {
                        c = c.Where(i => i.EREP_SUB_EMP_NUMBER == query.SubordinateEmployeeNumber);
                    }
                    if (query.ReportingMode != null)
                    {
                        c = c.Where(i => i.EREP_REPORTING_MODE == query.ReportingMode);
                    }

                    int totalcount = c.Count();
                    var result = c.OrderBy(i => i.EREP_SUP_EMP_NUMBER).Skip((pageNumber - 1) * pageSize).Include(h => h.HR_EMPLOYEE).Include(h=>h.HR_EMPLOYEE1).Take(pageSize).ToArray();
                    int localcount = result.Count();
                    var dto = new ResultDTO<HR_EMP_REPORTTO>() { Result = result, TotalResultCount = totalcount, LocalResultCount = localcount };
                    return dto;
                }
            }
            catch (Exception ex)
        ...
        public ApiAck CreateReporting(HR_EMP_REPORTTO new_Reporting)
        {
            try
            {
                using (var transaction = new TransactionScope())
                {
                    using (var ctx = new MDMEntities())
                    {
                        var emp_number = ctx.HR_EMPLOYEE.Find(new_Reporting.HR_EMPLOYEE.EMP_NUMBER);
                        var emp_number2=ctx.HR_EMPLOYEE.Find(new_Reporting.HR_EMPLOYEE1.EMP_NUMBER);
                        new_Reporting.HR_EMPLOYEE= emp_number;
                        new_Reporting.HR_EMPLOYEE1 = emp_number2;
                        ctx.HR_EMP_REPORTTO.Add(new_Reporting);
                        ctx.SaveChanges();
                    }
                    transaction.Complete();
                }
            ...
            {
                APIErrorLog.LogAPIError("FM100", string.Format("Unexpected error has occurred on creating new Reporting : {0} ", ex.Message));
                return new ApiAck { CallStatus = EApiCallStatus.Error, ReturnedMessage = ex.GetBaseException().Message };
            }
        }

        /// <summary>
        /// Return a Reporting record
        /// </summary>
        /// <param name="emp_number">The emp_number.</param>
        /// <param name="tn_id">The tn_id.</param>
        /// <returns>HR_EMP_TRAININGS.</returns>
        public HR_EMP_REPORTTO ReadReportingByKey(string emp_number,string emp_number2, Int16 reporting_mode)
        {
            try
            {
                using (var ctx = new MDMEntities())
                {
                    return ctx.HR_EMP_REPORTTO.Where(c => c.EREP_SUP_EMP_NUMBER==emp_number && c.EREP_SUB_EMP_NUMBER==emp_number2 && c.EREP_REPORTING_MODE==reporting_mode).FirstOrDefault();
                }
            }
            catch (Exception ex)
            {
                APIErrorLog.LogAPIError("FM100", string.Format("Unexpected error has occurred on read Reporting by key : {0} ", ex.Message));
                return null;
            }
        }

        /// <summary>
        /// Return all Reporting records
        /// </summary>
        /// <returns>HR_EMP_TRAININGS[].</returns>
        public HR_EMP_REPORTTO[] ReadAllReporting()
        {
            try
            {
                using (var ctx = new MDMEntities())
                {
                    return ctx.HR_EMP_REPORTTO.ToArray();
                }
            }
            catch (Exception ex)
            {
                APIErrorLog.LogAPIError("FM100", string.Format("Unexpected error has occurred on read all Reporting : {0} ", ex.Message));
                return new HR_EMP_REPORTTO[0];
            }
        }
        ...
        public ApiAck UpdateReporting(HR_EMP_REPORTTO modified_Reporting)
        {
            try
            {
                using (var transaction = new TransactionScope())
                {
                    using (var ctx = new MDMEntities())
                    {
                        HR_EMP_REPORTTO original_Reporting = ctx.HR_EMP_REPORTTO.Find(modified_Reporting.EREP_SUP_EMP_NUMBER, modified_Reporting.EREP_SUB_EMP_NUMBER, modified_Reporting.EREP_REPORTING_MODE);

                        if (original_Reporting != null)
                        {
                            ctx.Entry(original_Reporting).CurrentValues.SetValues(modified_Reporting);
                            ctx.SaveChanges();
                        }
                        else
                        {
                            ...
                            {
                                CallStatus = EApiCallStatus.ValidationFailed,
                                ReturnedMessage = string.Format("Reporting with Superior Employee Number :{0} Subordinate Employee Number:{1} Reporting mode:{2}was not found.", modified_Reporting.EREP_SUP_EMP_NUMBER,modified_Reporting.EREP_SUB_EMP_NUMBER,modified_Reporting.EREP_REPORTING_MODE)
                            };
                        }
                    }
                    transaction.Complete();
            ...
            {
                APIErrorLog.LogAPIError("FM100", string.Format("Unexpected error has occurred on update Reporting mode : {0} ", ex.Message));
                return new ApiAck { CallStatus = EApiCallStatus.Error, ReturnedMessage = ex.GetBaseException().Message };
            }
        }
        ...
        public ApiAck DeleteReporting(HR_EMP_REPORTTO delete_Reporting)
        {
            try
            {
                using (var transaction = new TransactionScope())
                {
                    using (var ctx = new MDMEntities())
                    {
                        HR_EMP_REPORTTO original_Reporting = ctx.HR_EMP_REPORTTO.Find(delete_Reporting.EREP_SUP_EMP_NUMBER, delete_Reporting.EREP_SUB_EMP_NUMBER, delete_Reporting.EREP_REPORTING_MODE);

                        if (original_Reporting != null)
                        {
                            ctx.HR_EMP_REPORTTO.Remove(original_Reporting);
                            ctx.SaveChanges();
                        }
                        else
                        {
                            ...
                            {
                                CallStatus = EApiCallStatus.ValidationFailed,
                                ReturnedMessage = string.Format("Reporting with Superior Employee Number :{0} Subordinate Employee Number:{1} Reporting mode:{2}was not found.", delete_Reporting.EREP_SUP_EMP_NUMBER,delete_Reporting.EREP_SUB_EMP_NUMBER,delete_Reporting.EREP_REPORTING_MODE)
                            };
                        }
                    }
                    transaction.Complete();
            ...
            {
                APIErrorLog.LogAPIError("FM100", string.Format("Unexpected error has occurred on delete Reporting : {0} ", ex.Message));
                return new ApiAck { CallStatus = EApiCallStatus.Error, ReturnedMessage = ex.GetBaseException().Message };
            }
        }

        /// <summary>
        /// Exists the Reporting.
        /// </summary>
        /// <param name="exist_Training">The exist_ training.</param>
        /// <returns><c>true</c> if XXXX, <c>false</c> otherwise.</returns>
        public bool ExistReporting(HR_EMP_REPORTTO exist_Reporting)
        {
            try
            {
                using (var ctx = new MDMEntities())
                {
                    HR_EMP_REPORTTO original_Reporting = ctx.HR_EMP_REPORTTO.Find(exist_Reporting.HR_EMPLOYEE.EMP_NUMBER, exist_Reporting.HR_EMPLOYEE1.EMP_NUMBER, exist_Reporting.EREP_REPORTING_MODE);
            ...
            {
                APIErrorLog.LogAPIError("FM100", string.Format("Unexpected error has occurred Reporting : {0} ", ex.Message));
                return false;
            }
        }
        ...
        /// <summary>
        /// Execute the query and return  Job Specification records
        /// </summary>
        /// <param name="query">The query.</param>
        /// <param name="pageSize">Size of the page.</param>
        /// <param name="pageNumber">The page number.</param>
        ...
        public ResultDTO<HR_EMP_JOBSPEC> QueryJobSpecification(JobSpecificationQuery query, int pageSize, int pageNumber)
        {
            try
            {
                using (var ctx = new MDMEntities())
                {
                    var c = ctx.HR_EMP_JOBSPEC.AsQueryable();
                    if (query.EmployeeNumber != null)
                    {
                        c = c.Where(i => i.EMP_NUMBER == query.EmployeeNumber);
                    }
                    if (query.JobDescriptionCategoryCode != null)
                    {
                        c = c.Where(i => i.JDCAT_CODE == query.JobDescriptionCategoryCode);
                    }

                    int totalcount = c.Count();
                    var result = c.OrderBy(i => i.EMP_NUMBER).Skip((pageNumber - 1) * pageSize).Include(h => h.HR_EMPLOYEE).Include(j=>j.HR_JD_CATEGORY).Take(pageSize).ToArray();
                    int localcount = result.Count();
                    var dto = new ResultDTO<HR_EMP_JOBSPEC>() { Result = result, TotalResultCount = totalcount, LocalResultCount = localcount };
                    return dto;
                }
            }
            catch (Exception ex)
        ...
        /// <summary>
        /// Create a new Job Specification record
        /// </summary>
        /// <param name="new_Training">The new_ training.</param>
        /// <returns>ApiAck.</returns>
        public ApiAck CreateJobSpecification(HR_EMP_JOBSPEC new_JobSpecification)
        {
            try
            {
                using (var transaction = new TransactionScope())
                {
                    using (var ctx = new MDMEntities())
                    {
                        var emp_number = ctx.HR_EMPLOYEE.Find(new_JobSpecification.HR_EMPLOYEE.EMP_NUMBER);
                        var jdcat_code = ctx.HR_JD_CATEGORY.Find(new_JobSpecification.HR_JD_CATEGORY.JDCAT_CODE);
                        new_JobSpecification.HR_EMPLOYEE = emp_number;
                        new_JobSpecification.HR_JD_CATEGORY = jdcat_code;
                        ctx.HR_EMP_JOBSPEC.Add(new_JobSpecification);
                        ctx.SaveChanges();
                    }
                    transaction.Complete();
                }
            ...
            {
                APIErrorLog.LogAPIError("FM100", string.Format("Unexpected error has occurred on creating new Job Specification : {0} ", ex.Message));
                return new ApiAck { CallStatus = EApiCallStatus.Error, ReturnedMessage = ex.GetBaseException().Message };
            }
        }

        /// <summary>
        /// Return a Job Specification record
        /// </summary>
        /// <param name="emp_number">The emp_number.</param>
        /// <param name="tn_id">The tn_id.</param>
        /// <returns>HR_EMP_TRAININGS.</returns>
        public HR_EMP_JOBSPEC ReadJobSpecificationByKey(string emp_number, string jdcat_code)
        {
            try
            {
                using (var ctx = new MDMEntities())
                {
                    return ctx.HR_EMP_JOBSPEC.Where(c => c.EMP_NUMBER == emp_number && c.JDCAT_CODE==jdcat_code).FirstOrDefault();
                }
            }
            catch (Exception ex)
            {
                APIErrorLog.LogAPIError("FM100", string.Format("Unexpected error has occurred on read Job Specification by key : {0} ", ex.Message));
                return null;
            }
        }

        /// <summary>
        /// Return all Job Specification records
        /// </summary>
        /// <returns>HR_EMP_TRAININGS[].</returns>
        public HR_EMP_JOBSPEC[] ReadAllJobSpecification()
        {
            try
            {
                using (var ctx = new MDMEntities())
                {
                    return ctx.HR_EMP_JOBSPEC.ToArray();
                }
            }
            catch (Exception ex)
            {
                APIErrorLog.LogAPIError("FM100", string.Format("Unexpected error has occurred on read all JobSpecification : {0} ", ex.Message));
                return new HR_EMP_JOBSPEC[0];
            }
        }

        /// <summary>
        /// Update Job Specification record
        /// </summary>
        /// <param name="modified_Training">The modified_ training.</param>
        /// <returns>ApiAck.</returns>
        public ApiAck UpdateJobSpecification(HR_EMP_JOBSPEC modified_JobSpecification)
        {
            try
            {
                using (var transaction = new TransactionScope())
                {
                    using (var ctx = new MDMEntities())
                    {
                        HR_EMP_JOBSPEC original_JobSpecification = ctx.HR_EMP_JOBSPEC.Find(modified_JobSpecification.EMP_NUMBER, modified_JobSpecification.JDCAT_CODE);

                        if (original_JobSpecification != null)
                        {
                            ctx.Entry(original_JobSpecification).CurrentValues.SetValues(modified_JobSpecification);
                            ctx.SaveChanges();
                        }
                        else
                        {
                            ...
                            {
                                CallStatus = EApiCallStatus.ValidationFailed,
                                ReturnedMessage = string.Format("Job Specification with employee number:{0} and Job description category code:{1} was not found.", modified_JobSpecification.EMP_NUMBER,modified_JobSpecification.JDCAT_CODE)
                            };
                        }
                    }
                    transaction.Complete();
            ...
            {
                APIErrorLog.LogAPIError("FM100", string.Format("Unexpected error has occurred on update Job Specification : {0} ", ex.Message));
                return new ApiAck { CallStatus = EApiCallStatus.Error, ReturnedMessage = ex.GetBaseException().Message };
            }
        }

        /// <summary>
        /// Delete Job Specification  record
        /// </summary>
        /// <param name="delete_Training">The delete_ training.</param>
        /// <returns>ApiAck.</returns>
        public ApiAck DeleteJobSpecification (HR_EMP_JOBSPEC delete_JobSpecification)
        {
            try
            {
                using (var transaction = new TransactionScope())
                {
                    using (var ctx = new MDMEntities())
                    {
                        HR_EMP_JOBSPEC original_JobSpecification  = ctx.HR_EMP_JOBSPEC.Find(delete_JobSpecification.EMP_NUMBER,delete_JobSpecification.JDCAT_CODE);

                        if (original_JobSpecification != null)
                        {
                            ctx.HR_EMP_JOBSPEC.Remove(original_JobSpecification);
                            ctx.SaveChanges();
                        }
                        else
                        {
                            ...
                            {
                                CallStatus = EApiCallStatus.ValidationFailed,
                                ReturnedMessage = string.Format("Job Specification with employee number:{0} and Job description category code:{1} was not found.", delete_JobSpecification.EMP_NUMBER,delete_JobSpecification.JDCAT_CODE)
                            };
                        }
                    }
                    transaction.Complete();
            ...
            {
                APIErrorLog.LogAPIError("FM100", string.Format("Unexpected error has occurred on delete Job Specification : {0} ", ex.Message));
                return new ApiAck { CallStatus = EApiCallStatus.Error, ReturnedMessage = ex.GetBaseException().Message };
            }
        }

        /// <summary>
        /// Exists the Job Specification
        /// </summary>
        /// <param name="exist_Training">The exist_ training.</param>
        /// <returns><c>true</c> if XXXX, <c>false</c> otherwise.</returns>
        public bool ExistJobSpecification(HR_EMP_JOBSPEC exist_JobSpecification)
        {
            try
            {
                using (var ctx = new MDMEntities())
                {
                    HR_EMP_JOBSPEC original_JobSpecification = ctx.HR_EMP_JOBSPEC.Find(exist_JobSpecification.HR_EMPLOYEE.EMP_NUMBER, exist_JobSpecification.HR_JD_CATEGORY.JDCAT_CODE);
            ...
            {
                APIErrorLog.LogAPIError("FM100", string.Format("Unexpected error has occurred Job Specification : {0} ", ex.Message));
                return false;
            }
        }
        ...
        public ResultDTO<HR_CURRENCY> QueryCurrency2(CurrencyHRQuery query, int pageSize, int pageNumber)
        {
            try
            {
                using (var ctx = new MDMEntities())
                {
                    var c = ctx.HR_CURRENCY.AsQueryable();
                    if (query.CurrencyID != null)
                    {
                        c = c.Where(i => i.CURRENCY_ID == query.CurrencyID);
                    }                    
                    ...
                    int totalcount = c.Count();
                    var result = c.OrderBy(i => i.CURRENCY_ID).Skip((pageNumber - 1) * pageSize).Take(pageSize).ToArray();
                    int localcount = result.Count();
                    var dto = new ResultDTO<HR_CURRENCY>() { Result = result, TotalResultCount = totalcount, LocalResultCount = localcount };
                    return dto;
                }
            }
            catch (Exception ex)
            {
                APIErrorLog.LogAPIError("FM100", string.Format("Unexpected error has occurred on Query Currency : {0} ", ex.Message));
                return null;
            }
        }
    #endregion 
        ...
        /// <summary>
        /// Execute the query and return Country records
        /// </summary>
        /// <param name="query">The query.</param>
        /// <param name="pageSize">Size of the page.</param>
        /// <param name="pageNumber">The page number.</param>
        /// <returns>ResultDTO&lt;HR_COUNTRY&gt;.</returns>
        public ResultDTO<HR_COUNTRY> QueryHRCountry(HRCountryQuery query, int pageSize, int pageNumber)
        {
            try
            {
                using (var ctx = new MDMEntities())
                {
                    var c = ctx.HR_COUNTRY.AsQueryable();
                    if (query.Code != null)
                    {
                        c = c.Where(i => i.COU_CODE == query.Code);
                    }
                    if (query.Name != null)
                    {
                        c = c.Where(i => i.COU_NAME.Contains(query.Name));
                    }

                    int totalcount = c.Count();
                    var result = c.OrderBy(i => i.COU_CODE).Skip((pageNumber - 1) * pageSize).Take(pageSize).ToArray();
                    int localcount = result.Count();
                    var dto = new ResultDTO<HR_COUNTRY>() { Result = result, TotalResultCount = totalcount, LocalResultCount = localcount };
                    return dto;
                }
            }
            catch (Exception ex)
            {
                APIErrorLog.LogAPIError("FM100", string.Format("Unexpected error has occurred on QueryHRCountry : {0} ", ex.Message));
                return null;
            }
        }
        #endregion 
        ...
        /// <param name="pageSize">Size of the page.</param>
        /// <param name="pageNumber">The page number.</param>
        /// <returns>ResultDTO&lt;HR_BANK&gt;.</returns>
        public ResultDTO<HR_BANK> QueryHRBank(HRBankQuery query, int pageSize, int pageNumber)
        {
            try
            {
                using (var ctx = new MDMEntities())
                {
                    var c = ctx.HR_BANK.AsQueryable();
                    if (query.Code != null)
                    {
                        c = c.Where(i => i.BANK_CODE == query.Code);
                    }
                    if (query.Name != null)
                    {
                        c = c.Where(i => i.BANK_NAME.Contains(query.Name));
                    }

                    int totalcount = c.Count();
                    var result = c.OrderBy(i => i.BANK_CODE).Skip((pageNumber - 1) * pageSize).Take(pageSize).ToArray();
                    int localcount = result.Count();
                    var dto = new ResultDTO<HR_BANK>() { Result = result, TotalResultCount = totalcount, LocalResultCount = localcount };
                    return dto;
                }
            }
            catch (Exception ex)
            {
                APIErrorLog.LogAPIError("FM100", string.Format("Unexpected error has occurred on QueryHRBank : {0} ", ex.Message));
                return null;
            }
        }
        #endregion 
        ...
        /// <summary>
        /// Execute the query and return Bank branch records
        /// </summary>
        /// <param name="query">The query.</param>
        /// <param name="pageSize">Size of the page.</param>
        /// <param name="pageNumber">The page number.</param>
        ...
        public ResultDTO<HR_BRANCH> QueryHRBankBranch(HRBankBranchQuery query, int pageSize, int pageNumber)
        {
            try
            {
                using (var ctx = new MDMEntities())
                {
                    var c = ctx.HR_BRANCH.AsQueryable();
                    if (query.Code != null)
                    {
                        c = c.Where(i => i.BBRANCH_CODE == query.Code);
                    }
                    if (query.Name != null)
                    {
                        c = c.Where(i => i.BBRANCH_NAME.Contains(query.Name));
                    }

                    int totalcount = c.Count();
                    var result = c.OrderBy(i => i.BANK_CODE).Skip((pageNumber - 1) * pageSize).Take(pageSize).ToArray();
                    int localcount = result.Count();
                    var dto = new ResultDTO<HR_BRANCH>() { Result = result, TotalResultCount = totalcount, LocalResultCount = localcount };
                    return dto;
                }
            }
            catch (Exception ex)
        ...
        /// <summary>
        /// Queries the type of the HR Language.
        /// </summary>
        /// <param name="query">The query.</param>
        /// <param name="pageSize">Size of the page.</param>
        /// <param name="pageNumber">The page number.</param>
        /// <returns>ResultDTO&lt;HR_LANGUAGE&gt;.</returns>
        public ResultDTO<HR_LANGUAGE> QueryHRLanguage(HRLanguageQuery query, int pageSize, int pageNumber)
        {
            try
            {
                using (var ctx = new MDMEntities())
                {
                    var c = ctx.HR_LANGUAGE.AsQueryable();

                    if (query.LangCode != null)
                    {
                        c = c.Where(i => i.LANG_CODE == query.LangCode);
                    }
                    if (query.Name != null)
                    {
                        c = c.Where(i => i.LANG_Name.Contains(query.Name));
                    }

                    int totalcount = c.Count();
                    var result = c.OrderBy(i => i.LANG_CODE).Skip((pageNumber - 1) * pageSize).Take(pageSize).ToArray();
                    int localcount = result.Count();
                    var dto = new ResultDTO<HR_LANGUAGE>() { Result = result, TotalResultCount = totalcount, LocalResultCount = localcount };
                    return dto;
                }
            }
            catch (Exception ex)
        ...
        /// <summary>
        /// Creates the type of the HR Language.
        /// </summary>
        /// <param name="new_HR_Language">Type of the new_HR_Language.</param>
        /// <returns>ApiAck.</returns>
        public ApiAck CreateHRLanguage(HR_LANGUAGE new_HR_Language)
        {
            try
            {
                using (var transaction = new TransactionScope())
                {
                    using (var ctx = new MDMEntities())
                    {
                        ctx.HR_LANGUAGE.Add(new_HR_Language);
                        ctx.SaveChanges();
                    }
                    transaction.Complete();
                }
            ...
            {
                APIErrorLog.LogAPIError("FM100", string.Format("Unexpected error has occurred on creating new HR Language : {0} ", ex.Message));
                return new ApiAck { CallStatus = EApiCallStatus.Error, ReturnedMessage = ex.GetBaseException().Message };
            }
        }

        /// <summary>
        /// Reads the HR Language typeby key.
        /// </summary>
        /// <param name="HRLanguage">Type of the Language.</param>
        /// <returns>HR_LANGUAGE.</returns>
        public HR_LANGUAGE ReadHRLanguagebyKey(string HRLanguage)
        {
            try
            {
                using (var ctx = new MDMEntities())
                {
                    return ctx.HR_LANGUAGE.Where(c => c.LANG_CODE == HRLanguage).FirstOrDefault();
                }
            }
            catch (Exception ex)
            {
                APIErrorLog.LogAPIError("FM100", string.Format("Unexpected error has occurred on ReadHRLanguagebyKey : {0} ", ex.Message));
                return null;
            }
        }

        /// <summary>
        /// Return all HR Language records
        /// </summary>
        /// <param name="pageSize">Size of the page.</param>
        /// <param name="pageNumber">The page number.</param>
        /// <returns>ResultDTO&lt;HR_LANGUAGE&gt;.</returns>
        public ResultDTO<HR_LANGUAGE> ReadAllHRLanguage(int pageSize, int pageNumber)
        {
            try
            {
                using (var ctx = new MDMEntities())
                {
                    int totalcount = ctx.HR_LANGUAGE.Count();
                    var result = ctx.HR_LANGUAGE.OrderBy(i => i.LANG_CODE).Skip((pageNumber - 1) * pageSize).Take(pageSize).ToArray();
                    int localcount = result.Count();
                    var dto = new ResultDTO<HR_LANGUAGE>() { Result = result, TotalResultCount = totalcount, LocalResultCount = localcount };
                    return dto;
                }
            ...
            {
                APIErrorLog.LogAPIError("FM100", string.Format("Unexpected error has occurred on ReadAllHRLanguage : {0} ", ex.Message));
                return new ResultDTO<HR_LANGUAGE>();
            }
        }

        /// <summary>
        /// Execute the query and Return HR Language records
        /// </summary>
        /// <returns>IQueryable&lt;HR_LANGUAGE&gt;.</returns>
        public IQueryable<HR_LANGUAGE> ReadHRLanguage()
        {
            try
            {
                using (var ctx = new MDMEntities())
                {
                    return ctx.HR_LANGUAGE.AsQueryable();
                }
            }
            catch (Exception ex)
            {
                APIErrorLog.LogAPIError("FM100", string.Format("Unexpected error has occurred on ReadHRLanguage : {0} ", ex.Message));
                return null;
            }
        }

        /// <summary>
        /// Update HR Language record
        /// </summary>
        /// <param name="modifiedHRLanguage">Type of the modified HR Language.</param>
        /// <returns>ApiAck.</returns>
        public ApiAck UpdateHRLanguage(HR_LANGUAGE modifiedHRLanguage)
        {
            try
            {
                using (var transaction = new TransactionScope())
                {
                    using (var ctx = new MDMEntities())
                    {
                        HR_LANGUAGE original = ctx.HR_LANGUAGE.Find(modifiedHRLanguage.LANG_CODE);

                        if (original != null)
                        {
                            ctx.Entry(original).CurrentValues.SetValues(modifiedHRLanguage);
                            ctx.SaveChanges();
                        }
                        else
                        {
                            return new ApiAck
                            {
                                CallStatus = EApiCallStatus.ValidationFailed,
                                ReturnedMessage =
                                    string.Format("HR Language with Language Code:{0} was not found.", modifiedHRLanguage.LANG_CODE)
                            };
                        }
                    }
                    transaction.Complete();
            ...
            catch (DbEntityValidationException dbEx)
            {
                ...
                APIErrorLog.LogAPIError("FM100", string.Format("Unexpected error has occurred on creating new HR Language : {0} ", errorMsg));
                return new ApiAck { CallStatus = EApiCallStatus.Error, ReturnedMessage = errorMsg };
            }
            catch (Exception ex)
            {
                APIErrorLog.LogAPIError("FM100", string.Format("Unexpected error has occurred on HR Language : {0} ", ex.Message));
                return new ApiAck { CallStatus = EApiCallStatus.Error, ReturnedMessage = ex.GetBaseException().Message };
            }
        }

        /// <summary>
        /// Delete HR Language record
        /// </summary>
        /// <param name="deletingAccountType">Type of the deleting HR Language.</param>
        /// <returns>ApiAck.</returns>
        public ApiAck DeleteHRLanguage(HR_LANGUAGE deletingHRLanguage)
        {
            try
            {
                using (var transaction = new TransactionScope())
                {
                    using (var ctx = new MDMEntities())
                    {
                        HR_LANGUAGE original = ctx.HR_LANGUAGE.Find(deletingHRLanguage.LANG_CODE);
                        ...
                        else
                        {
                            return new ApiAck
                            {
                                CallStatus = EApiCallStatus.ValidationFailed,
                                ReturnedMessage =
                                    string.Format("HR Language with Language Code:{0}  was not found.", deletingHRLanguage.LANG_CODE)
                            };
                        }
                    }
                    transaction.Complete();
            ...
            {
                APIErrorLog.LogAPIError("FM100", string.Format("Unexpected error has occurred on DeleteHRLanguage) : {0} ", ex.Message));
                return new ApiAck { CallStatus = EApiCallStatus.Error, ReturnedMessage = ex.GetBaseException().Message };
            }
        }
        ...
        /// </summary>
        /// <param name="existsHRLanguage">Type of the exists account.</param>
        /// <returns><c>true</c> if XXXX, <c>false</c> otherwise.</returns>
        public bool HRLanguageExists(HR_LANGUAGE existsHRLanguage)
        {
            try
            {
                using (var ctx = new MDMEntities())
                {
                    FIN_AccountType original = ctx.FIN_AccountType.Find(existsHRLanguage.LANG_CODE);
            ...
            {
                APIErrorLog.LogAPIError("FM100", string.Format("Unexpected error has occurred HR Language : {0} ", ex.Message));
                return false;
            }
        }
        ...
        /// <summary>
        /// Queries the type of the HR Employee Language.
        /// </summary>
        /// <param name="query">The query.</param>
        /// <param name="pageSize">Size of the page.</param>
        /// <param name="pageNumber">The page number.</param>
        ...
        public ResultDTO<HR_EMP_LANGUAGE> QueryHREmpLanguage(HREmpLanguageQuery query, int pageSize, int pageNumber)
        {
            try
            {
                using (var ctx = new MDMEntities())
                {
                    var c = ctx.HR_EMP_LANGUAGE.AsQueryable();
                    ...
                    int totalcount = c.Count();
                    var result = c.OrderBy(i => i.EMP_NUMBER).Skip((pageNumber - 1) * pageSize).Include(s => s.HR_EMPLOYEE).Include(s => s.HR_LANGUAGE).Take(pageSize).ToArray();
                    int localcount = result.Count();
                    var dto = new ResultDTO<HR_EMP_LANGUAGE>() { Result = result, TotalResultCount = totalcount, LocalResultCount = localcount };
                    return dto;
                }
            }
            catch (Exception ex)
            {
                APIErrorLog.LogAPIError("FM100", string.Format("Unexpected error has occurred on QueryHREmpLanguage : {0} ", ex.Message));
                return null;
            }
        }
        ...
        /// </summary>
        /// <param name="new_HR_Emp_Language">Type of the new_HR_Emp_Language.</param>
        /// <returns>ApiAck.</returns>
        public ApiAck CreateHREmpLanguage(HR_EMP_LANGUAGE new_HR_Emp_Language)
        {
            try
            {
                using (var transaction = new TransactionScope())
                {
                    using (var ctx = new MDMEntities())
                    {
                        var language = ctx.HR_LANGUAGE.Find(new_HR_Emp_Language.LANG_CODE);
                        var employee = ctx.HR_EMPLOYEE.Find(new_HR_Emp_Language.EMP_NUMBER);
                        new_HR_Emp_Language.HR_EMPLOYEE = employee;
                        new_HR_Emp_Language.HR_LANGUAGE = language;
                        ctx.HR_EMP_LANGUAGE.Add(new_HR_Emp_Language);
                        ctx.SaveChanges();
                    }
                    transaction.Complete();
                }
            ...
            {
                APIErrorLog.LogAPIError("FM100", string.Format("Unexpected error has occurred on creating new HREmpLanguage : {0} ", ex.Message));
                return new ApiAck { CallStatus = EApiCallStatus.Error, ReturnedMessage = ex.GetBaseException().Message };
            }
        }

        /// <summary>
        /// Reads the HR Employee  Language by key.
        /// </summary>
        /// <param name="EmployeeCode">Employee Code.</param>
        /// <param name="LanguageCode">Language Code.</param>
        /// <returns>HR_EMP_LANGUAGE.</returns>
        public HR_EMP_LANGUAGE ReadHREmpLanguagebyKey(string EmployeeCode, string LanguageCode)
        {
            try
            {
                using (var ctx = new MDMEntities())
                {
                    return ctx.HR_EMP_LANGUAGE.Where(c => c.EMP_NUMBER == EmployeeCode && c.LANG_CODE == LanguageCode).FirstOrDefault();
                }
            }
            catch (Exception ex)
            {
                APIErrorLog.LogAPIError("FM100", string.Format("Unexpected error has occurred on ReadHREmpLanguagebyKey : {0} ", ex.Message));
                return null;
            }
        }

        /// <summary>
        /// Return all HR Employee Language records
        /// </summary>
        /// <param name="pageSize">Size of the page.</param>
        /// <param name="pageNumber">The page number.</param>
        /// <returns>ResultDTO&lt;HR_EMP_LANGUAGE&gt;.</returns>
        public ResultDTO<HR_EMP_LANGUAGE> ReadAllHREmpLanguage(int pageSize, int pageNumber)
        {
            try
            {
                ...
                {
                    int totalcount = ctx.HR_LANGUAGE.Count();
                    var result = ctx.HR_EMP_LANGUAGE.OrderBy(i => i.EMP_NUMBER).Skip((pageNumber - 1) * pageSize).Take(pageSize).ToArray();
                    int localcount = result.Count();
                    var dto = new ResultDTO<HR_EMP_LANGUAGE>() { Result = result, TotalResultCount = totalcount, LocalResultCount = localcount };
                    return dto;
                }
            ...
            {
                APIErrorLog.LogAPIError("FM100", string.Format("Unexpected error has occurred on ReadAllHRLanguage : {0} ", ex.Message));
                return new ResultDTO<HR_EMP_LANGUAGE>();
            }
        }

        /// <summary>
        /// Execute the query and Return HR Employee Language records
        /// </summary>
        /// <returns>IQueryable&lt;HR_EMP_LANGUAGE&gt;.</returns>
        public IQueryable<HR_EMP_LANGUAGE> ReadHREmpLanguage()
        {
            try
            {
                using (var ctx = new MDMEntities())
                {
                    return ctx.HR_EMP_LANGUAGE.AsQueryable();
                }
            }
            catch (Exception ex)
            {
                APIErrorLog.LogAPIError("FM100", string.Format("Unexpected error has occurred on ReadHREmpLanguage : {0} ", ex.Message));
                return null;
            }
        }
        ...
        /// </summary>
        /// <param name="modifiedHREmpLanguage">Type of the modified account sub.</param>
        /// <returns>ApiAck.</returns>
        public ApiAck UpdateHREmpLanguage(HR_EMP_LANGUAGE modifiedHREmpLanguage)
        {
            try
            {
                using (var transaction = new TransactionScope())
                {
                    using (var ctx = new MDMEntities())
                    {
                        HR_EMP_LANGUAGE original = ctx.HR_EMP_LANGUAGE.Find(modifiedHREmpLanguage.EMP_NUMBER, modifiedHREmpLanguage.LANG_CODE);

                        if (original != null)
                        {
                            ctx.Entry(original).CurrentValues.SetValues(modifiedHREmpLanguage);
                            ctx.SaveChanges();
                        }
                        else
                        {
                            return new ApiAck
                            {
                                CallStatus = EApiCallStatus.ValidationFailed,
                                ReturnedMessage =
                                    string.Format("HR EmpLanguage with Employee Code:{0} Language Code:{1}  was not found.", modifiedHREmpLanguage.EMP_NUMBER, modifiedHREmpLanguage.LANG_CODE)
                            };
                        }
                    }
                    transaction.Complete();
            ...
            {
                APIErrorLog.LogAPIError("FM100", string.Format("Unexpected error has occurred on UpdateHREmpLanguage : {0} ", ex.Message));
                return new ApiAck { CallStatus = EApiCallStatus.Error, ReturnedMessage = ex.GetBaseException().Message };
            }
        }

        /// <summary>
        /// Delete HR Employee Language record
        /// </summary>
        ...
        public ApiAck DeleteHREmpLanguage(HR_EMP_LANGUAGE deletingHREmpLanguage)
        {
            try
            {
                using (var transaction = new TransactionScope())
                {
                    using (var ctx = new MDMEntities())
                    {
                        HR_EMP_LANGUAGE original = ctx.HR_EMP_LANGUAGE.Find(deletingHREmpLanguage.EMP_NUMBER, deletingHREmpLanguage.LANG_CODE);
                        ...
                        else
                        {
                            return new ApiAck
                            {
                                CallStatus = EApiCallStatus.ValidationFailed,
                                ReturnedMessage =
                                    string.Format("HR Emp Language with Emp Code:{0} Lang Code:{1}  was not found.", deletingHREmpLanguage.EMP_NUMBER, deletingHREmpLanguage.LANG_CODE)
                            };
                        }
                    }
                    transaction.Complete();
            ...
            {
                APIErrorLog.LogAPIError("FM100", string.Format("Unexpected error has occurred on DeleteHREmpLanguage) : {0} ", ex.Message));
                return new ApiAck { CallStatus = EApiCallStatus.Error, ReturnedMessage = ex.GetBaseException().Message };
            }
        }
        ...
        /// </summary>
        /// <param name="existsHREmpLanguage">Type of the exists account sub.</param>
        /// <returns><c>true</c> if XXXX, <c>false</c> otherwise.</returns>
        public bool HREmpLanguageExists(HR_EMP_LANGUAGE existsHREmpLanguage)
        {
            try
            {
                using (var ctx = new MDMEntities())
                {
                    HR_EMP_LANGUAGE original = ctx.HR_EMP_LANGUAGE.Find(existsHREmpLanguage.EMP_NUMBER, existsHREmpLanguage.LANG_CODE);
            ...
            {
                APIErrorLog.LogAPIError("FM100", string.Format("Unexpected error has occurred HR Emp Language : {0} ", ex.Message));
                return false;
            }
        }
