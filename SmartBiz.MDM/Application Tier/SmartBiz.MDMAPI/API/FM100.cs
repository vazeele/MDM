...
namespace SmartBiz.MDMAPI.API
{
    ...
    public class FM100 : IDisposable
    {
        ...
        public ResultDTO<FIN_CostCenter> QueryCostCenter(CostCenterQuery query, int pageSize, int pageNumber)
        {
            try
            {
                using (var ctx = new MDMEntities())
                {
                    ...
                    {
                        c = c.Where(i => i.CostCenterDescription.Contains(query.CostCenterDescription));
                    }
                    int totalcount = c.Count();
        ...
        /// <summary>
        /// Queries the Customer or Supplier.
        /// </summary>
        /// <param name="query">The query.</param>
        /// <param name="pageSize">Size of the page.</param>
        /// <param name="pageNumber">The page number.</param>
        /// <returns>ResultDTO&lt;FIN_CustomerSupplier_Info&gt;.</returns>
        public ResultDTO<FIN_CustomerSupplier_Info> QueryCustomerSupplier(CustomerSupplierQuery query, int pageSize, int pageNumber)
        {
            try
            {
                using (var ctx = new MDMEntities())
                {
                    var c = ctx.FIN_CustomerSupplier_Info.AsQueryable();

                    if (query.CusSupFlag != null)
                    {
                        c = c.Where(i => i.CustSupFlag == query.CusSupFlag);
                    }
                    if (query.CusSupCode != null)
                    {
                        c = c.Where(i => i.CusSupCode == query.CusSupCode);
                    }

                    if (query.Name != null)
                    {
                        c = c.Where(i => i.CustSupName.Contains(query.Name));
                    }

                    int totalcount = c.Count();
                    var result = c.OrderBy(i => i.CustSupFlag).Skip((pageNumber - 1) * pageSize).Take(pageSize).ToArray();
                    int localcount = result.Count();
                    var dto = new ResultDTO<FIN_CustomerSupplier_Info>() { Result = result, TotalResultCount = totalcount, LocalResultCount = localcount };
                    return dto;
                }
            }
            catch (Exception ex)
            {
                APIErrorLog.LogAPIError("FM100", string.Format("Unexpected error has occurred on QueryCustomerSupplier : {0} ", ex.Message));
                return null;
            }
        }
        ...
        public ResultDTO<FIN_CustomerSupplierBank> QueryCustomerSupplierBank(CustomerSupplierBankQuery query, int pageSize, int pageNumber)
        {
            try
            {
                using (var ctx = new MDMEntities())
                {
                    var c = ctx.FIN_CustomerSupplierBank.AsQueryable();
                    ...
                    if (query.BankCode != null)
                    {
                        c = c.Where(i => i.BankCode.Contains(query.BankCode));
                    }

                    if (query.BranchCode != null)
                    {
                        c = c.Where(i => i.BranchCode == query.BranchCode);
                    }
                    int totalcount = c.Count();
                    var result = c.OrderBy(i => i.CusSupCode).Skip((pageNumber - 1) * pageSize).Take(pageSize).ToArray();
                    int localcount = result.Count();
                    var dto = new ResultDTO<FIN_CustomerSupplierBank>() { Result = result, TotalResultCount = totalcount, LocalResultCount = localcount };
                    return dto;
                }
            }
            catch (Exception ex)
            {
                APIErrorLog.LogAPIError("FM100", string.Format("Unexpected error has occurred on QueryCustomerSupplierBank : {0} ", ex.Message));
                return null;
            }
        }
            ...
            try
            {
                using (var transaction = new TransactionScope())
                {
                    using (var ctx = new MDMEntities())
                    {
                        ...
                        var cusSupInfo = ctx.FIN_CustomerSupplier_Info.Find(new_FIN_CustomerSupplierBank.FIN_CustomerSupplier_Info.CustSupFlag, new_FIN_CustomerSupplierBank.FIN_CustomerSupplier_Info.CusSupCode);
                        new_FIN_CustomerSupplierBank.FIN_CustomerSupplier_Info = cusSupInfo;

                        var cusSupBank = ctx.FIN_BankBranch.Find(new_FIN_CustomerSupplierBank.FIN_BankBranch.BankCode, new_FIN_CustomerSupplierBank.FIN_BankBranch.BankBranchCode);
                        new_FIN_CustomerSupplierBank.FIN_BankBranch = cusSupBank;
        ...
        #endregion ProcessControl

        #region REPORTS
        public List<string> getReportNames(string loginuser)
        {
            ...
            try
            {
                var connection = System.Configuration.ConfigurationManager.ConnectionStrings["ReportsDB"].ToString();
...
where UserName IN('" + loginuser + "','"+loginuser.Split('\\')[1]+"') and rolename='Report builder' and E.Name NOT IN('','Report Parts') ", conn);
