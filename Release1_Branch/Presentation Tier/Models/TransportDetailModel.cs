...
namespace SmartBiz.MDM.Presentation
{
    ...
    {
        ObservableCollection<HR_EMP_TRANSPORT> TransportDetailInformation; //These will be shown in the datagrid
        TransportDetail TransportDetail; //Reference to Transaction Interface
        HR_EMP_TRANSPORT EditingTransportDetailInfomation; //Stores what we're editing right now, Incase we need to cancel the edit
        ...
        {           
          ...
          //  TransportDetail.TransportDetailSearchControl.ResultsGrid.AutoGeneratingColumn += dgv_TransportDetail_AutoGeneratingColumn;

            TransportDetail.TransportDetailSearchControl.ResultsGrid.SelectionChanged += dgv_TransportDetail_SelectionChanged;
        ...
        void dgv_TransportDetail_AutoGeneratingColumn(object sender, DataGridAutoGeneratingColumnEventArgs e)
        {
            if (!Attribute.IsDefined(typeof(HR_EMP_TRANSPORT).GetProperty(e.PropertyName), typeof(DataMemberAttribute)))
                e.Cancel = true;
            e.Column.Header = Helper.FormatHeaders(e.PropertyName, false);
        }
        
        //When the user selects a row in the datagrid
        void dgv_TransportDetail_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
        ...
        void TransportDetailGrid_SourceUpdated(object sender, DataTransferEventArgs e)
        {
            if (TransportDetail.TransportDetailGrid.DataContext == TransportDetailInformation)
            {
                ...
                foreach (TabItem t in TransportDetail.TbPage.Items)
                {
                    if (!t.IsSelected)
                        t.IsEnabled = false;
                }
