...
{
    class UnitDefinitionModel
    {
        ObservableCollection<ERP_UnitDefinition> UnitDefinitionInformation; //These will be shown in the datagrid
        Unit unit; //Reference to Transaction Interface
        ERP_UnitDefinition EditingUnitDefinition; //Stores what we're editing right now, Incase we need to cancel the edit
        public UnitDefinitionModel(Unit unit)
        {
            this.unit = unit;
            unit.UDSearchControl.Search = Search;
            unit.MUCDropDown.Search = UnitDefinitionModel.Search;
           // unit.UDSearchControl.ResultsGrid.AutoGeneratingColumn += dgv_UnitDefinition_AutoGeneratingColumn;
            unit.UDSearchControl.ResultsGrid.SelectionChanged += dgv_UnitDefinition_SelectionChanged;
        ...
        void dgv_UnitDefinition_AutoGeneratingColumn(object sender, DataGridAutoGeneratingColumnEventArgs e)
        {
            if (!Attribute.IsDefined(typeof(ERP_UnitDefinition).GetProperty(e.PropertyName), typeof(DataMemberAttribute)))
                e.Cancel = true;
        }
        ...
        void dgv_UnitDefinition_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
        ...
        void UnitDefinitionGrid_SourceUpdated(object sender, DataTransferEventArgs e)
        {
