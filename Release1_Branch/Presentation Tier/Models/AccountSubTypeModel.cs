...
{
    class AccountSubTypeModel
    {
        ObservableCollection<FIN_AccountSubType> LastAccountInformation; //These will be shown in the datagrid
        Accounts account; //Reference to Transaction Interface
        FIN_AccountSubType EditingAccountInfo; //Stores what we're editing right now, Incase we need to cancel the edit
        public AccountSubTypeModel(Accounts account)
        {
            this.account = account;
            //account.AccountSubTypeSearchControl.ResultsGrid.AutoGeneratingColumn += dgv_AccountSubType_AutoGeneratingColumn;

            account.AccountSubTypeDropDown.Search = AccountTypeSearchModel.AccountTypeSearch;

            account.AccountSubTypeSearchControl.ResultsGrid.SelectionChanged += dgv_AccountSubType_SelectionChanged;
        ...
        void dgv_AccountSubType_AutoGeneratingColumn(object sender, DataGridAutoGeneratingColumnEventArgs e)
        {
            if (!Attribute.IsDefined(typeof(FIN_AccountSubType).GetProperty(e.PropertyName), typeof(DataMemberAttribute)))
                e.Cancel = true;
        ...
        void AccountSubTypeGrid_SourceUpdated(object sender, DataTransferEventArgs e)
        {
