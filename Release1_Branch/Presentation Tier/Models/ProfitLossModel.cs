...
namespace SmartBiz.MDM.Presentation
{
    ...
    {
        ObservableCollection<FIN_ProfitLostType> LastAccountInformation; //These will be shown in the datagrid
        Accounts account; //Reference to Account Interface
        FIN_ProfitLostType EditingLastAccountInfo; //Stores what we're editing right now, Incase we need to cancel the edit
        public ProfitLossModel(Accounts account)
        {
            this.account = account;
            //To hide unnecessary columns
           // account.ProfitLossSearchControl.ResultsGrid.AutoGeneratingColumn += dgv_ProfitLoss_AutoGeneratingColumn;
            account.ProfitLossSearchControl.ResultsGrid.SelectionChanged += dgv_ProfitLoss_SelectionChanged;
        ...
        void dgv_ProfitLoss_AutoGeneratingColumn(object sender, DataGridAutoGeneratingColumnEventArgs e)
        {
            if (!Attribute.IsDefined(typeof(FIN_ProfitLostType).GetProperty(e.PropertyName), typeof(DataMemberAttribute)))
                e.Cancel = true;
        ...
        void ProfitLossGrid_SourceUpdated(object sender, DataTransferEventArgs e)
        {
        ...
        {   //Bind the original collection when the Add operation is complete      
            account.ProfitLostTypeGrid.DataContext = LastAccountInformation;
            //account.ProfitLossSearchControl.DataContext = LastAccountInformation;
