...
namespace SmartBiz.MDM.Presentation
{
    class CurrencyModel
    {
        ObservableCollection<FIN_Currency> CurrencyInformation; //These will be shown in the datagrid
        Currency Currency; //Reference to Transaction Interface
        FIN_Currency EditingCurrencyInfomation; //Stores what we're editing right now, Incase we need to cancel the edit
        public CurrencyModel(Currency Currency)
        {
           ...
           // Currency.CurrencySearchControl.ResultsGrid.AutoGeneratingColumn += dgv_Currency_AutoGeneratingColumn;

            Currency.CurrencySearchControl.ResultsGrid.SelectionChanged += dgv_Currency_SelectionChanged;
        ...
        void dgv_Currency_AutoGeneratingColumn(object sender, DataGridAutoGeneratingColumnEventArgs e)
        {
            if (!Attribute.IsDefined(typeof(FIN_Currency).GetProperty(e.PropertyName), typeof(DataMemberAttribute)))
                e.Cancel = true;
        ...
        void CurrencyGrid_SourceUpdated(object sender, DataTransferEventArgs e)
        {
