...
namespace SmartBiz.MDM.Presentation
{
    ...
    {
        ObservableCollection<FIN_BankAccount> BankAccounts; //These will be shown in the datagrid
        Bank bank; //Reference to Transaction Interface
        FIN_BankAccount EditingBankAccount; //Stores what we're editing right now, Incase we need to cancel the edit
        public BankAccountModel(Bank bank)
        {
            ...
            //bank.BankAccountSearchControl.ResultsGrid.AutoGeneratingColumn += ResultsGrid_AutoGeneratingColumn;
            bank.BankAccountSearchControl.ResultsGrid.SelectionChanged += SearchControl_SelectionChanged;
        ...
        {
            if (!Attribute.IsDefined(typeof(FIN_BankAccount).GetProperty(e.PropertyName), typeof(DataMemberAttribute)))
                e.Cancel = true;
