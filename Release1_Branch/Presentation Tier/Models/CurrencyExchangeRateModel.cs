...
namespace SmartBiz.MDM.Presentation
{
    ...
    {
        ObservableCollection<FIN_CurrencyExchangeRate> CurrencyExchangeRateInformation; //These will be shown in the datagrid
        Currency Currency; //Reference to Transaction Interface
        FIN_CurrencyExchangeRate EditingCurrencyExchangeRateInfomation; //Stores what we're editing right now, Incase we need to cancel the edit
        public CurrencyExchangeRateModel(Currency Currency)
        {
            ...
            //Currency.CurrencyExchangeRateSearchControl.ResultsGrid.AutoGeneratingColumn += dgv_CurrencyExchangeRate_AutoGeneratingColumn;

            Currency.CurrencyExchangeRateSearchControl.ResultsGrid.SelectionChanged += dgv_CurrencyExchangeRate_SelectionChanged;
        ...
        void dgv_CurrencyExchangeRate_AutoGeneratingColumn(object sender, DataGridAutoGeneratingColumnEventArgs e)
        {
            if (!Attribute.IsDefined(typeof(FIN_CurrencyExchangeRate).GetProperty(e.PropertyName), typeof(DataMemberAttribute)))
                e.Cancel = true;
            e.Column.Header = Helper.FormatHeaders(e.PropertyName, false);
        }        
        //When the user selects a row in the datagrid
        void dgv_CurrencyExchangeRate_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
        ...
        void CurrencyExchangeRateGrid_SourceUpdated(object sender, DataTransferEventArgs e)
        {
