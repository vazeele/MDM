...
{
    class ControlledTransactionModel
    {
        ObservableCollection<FIN_ControledTransaction> ControlledTransactions; //These will be shown in the datagrid
        Transaction transaction; //Reference to Transaction Interface
        FIN_ControledTransaction EditingControlledTransaction; //Stores what we're editing right now, Incase we need to cancel the edit
        public ControlledTransactionModel(Transaction transaction)
        {
          ...
          //  transaction.CTSearchControl.ResultsGrid.AutoGeneratingColumn += dgv_ControlledTransaction_AutoGeneratingColumn;
            
            transaction.CTSearchControl.ResultsGrid.SelectionChanged += dgv_ControlledTransaction_SelectionChanged;
        ...
        void dgv_ControlledTransaction_AutoGeneratingColumn(object sender, DataGridAutoGeneratingColumnEventArgs e)
        {
            if (!Attribute.IsDefined(typeof(FIN_ControledTransaction).GetProperty(e.PropertyName), typeof(DataMemberAttribute)))
                e.Cancel = true;
            e.Column.Header = Helper.FormatHeaders(e.PropertyName, false);
        }
      
        //When the user selects a row in the datagrid
        void dgv_ControlledTransaction_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
        ...
        void ControlledTransactionGrid_SourceUpdated(object sender, DataTransferEventArgs e)
        {
