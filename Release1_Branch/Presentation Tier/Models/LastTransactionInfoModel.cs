...
{
    class LastTransactionInfoModel
    {
        ObservableCollection<ERP_LastTransactionInfo> LastTransactionInformation; //These will be shown in the datagrid
        Transaction transaction; //Reference to Transaction Interface
        ERP_LastTransactionInfo EditingLastTransactionInfo; //Stores what we're editing right now, Incase we need to cancel the edit
        public LastTransactionInfoModel(Transaction transaction)
        {
           ...
           //transaction.LTISearchControl.ResultsGrid.AutoGeneratingColumn += dgv_LastTransactionInfo_AutoGeneratingColumn;
            
            transaction.LTISearchControl.ResultsGrid.SelectionChanged += dgv_LastTransactionInfo_SelectionChanged;
        ...
        {
            if (!Attribute.IsDefined(typeof(ERP_LastTransactionInfo).GetProperty(e.PropertyName), typeof(DataMemberAttribute)))
                e.Cancel = true;
        ...
        void LastTransactionInfoGrid_SourceUpdated(object sender, DataTransferEventArgs e)
        {
